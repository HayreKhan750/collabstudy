...
export function MessageBubble({
  ...
  const bubbleColorClass = isOwnMessage
    ...
    : 'bg-white dark:bg-[#1B2130] text-slate-900 dark:text-slate-100 shadow-sm dark:shadow-[0_2px_8px_rgba(0,0,0,0.3)] border border-slate-100 dark:border-white/[0.06]';
          ...
          {isEditing ? (
            <div className="flex flex-col gap-1 w-full min-w-[240px]">
              <textarea
                ...
                className="w-full bg-slate-50 dark:bg-[#141821] border border-violet-400/50 dark:border-violet-500/40 rounded-xl px-3 py-2 text-sm text-slate-900 dark:text-slate-100 resize-none focus:outline-none focus:ring-2 focus:ring-violet-500/40 transition-all duration-200"
                rows={2}
                autoFocus
                disabled={editSaving}
              />
              ...
              <div className="flex items-center gap-2 text-xs">
                <span className="text-slate-400 dark:text-slate-500">
                  Press{' '}
                  <kbd className="px-1 py-0.5 bg-slate-100 dark:bg-slate-700 rounded text-slate-700 dark:text-slate-300 text-[10px]">
                    Enter
                  </kbd>{' '}
                  to save,{' '}
                  <kbd className="px-1 py-0.5 bg-slate-100 dark:bg-slate-700 rounded text-slate-700 dark:text-slate-300 text-[10px]">
                ...
                <button
                  onClick={onEditSave}
                  disabled={editSaving}
                  className="ml-auto px-2.5 py-1 bg-gradient-to-r from-violet-600 to-indigo-600 hover:opacity-90 text-white rounded-lg disabled:opacity-50 transition-all duration-200 text-xs font-medium"
                >
                  {editSaving ? 'Saving…' : 'Save'}
                </button>
                <button
                  onClick={onEditCancel}
                  className="px-2.5 py-1 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 rounded-lg transition-all duration-200 text-xs font-medium"
                >
                  Cancel
                </button>
              </div>
                ...
                >
                  <span
                    className={`
                      ...
                      ${isOwnMessage ? 'bg-violet-600 -rotate-45 origin-bottom-left' : 'bg-white dark:bg-[#1B2130] rotate-45 origin-bottom-right'}
                    `}
                  />
                </span>
              )}
