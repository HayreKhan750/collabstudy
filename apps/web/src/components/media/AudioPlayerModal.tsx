...
export function AudioPlayerModal({ src, name, onClose }: AudioPlayerModalProps) {
  ...
  return (
    ...
    >
      <div
        className="bg-gray-800 border border-white/10 rounded-2xl shadow-2xl w-full max-w-sm mx-4 p-6 flex flex-col gap-5"
      ...
      >
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="text-white font-semibold text-sm truncate">{name || 'Audio'}</p>
              <p className="text-gray-400 text-xs">{formatDuration(currentTime)} / {formatDuration(duration)}</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex items-end justify-center gap-0.5 h-10 overflow-hidden">
          {Array.from({ length: 28 }).map((_, i) => (
            <div
              key={i}
              className={`w-1 rounded-full transition-all ${playing ? 'bg-blue-400' : 'bg-gray-600'}`}
              style={{
                height: playing
        ...
        <div className="flex flex-col gap-1">
          <input
            ...
            value={currentTime}
            onChange={seek}
            className="w-full h-1.5 rounded-full appearance-none bg-gray-600 accent-blue-500 cursor-pointer"
            aria-label="Seek"
          />
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 w-28">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-gray-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M15.536 8.464a5 5 0 010 7.072M12 6v12m-3.536-9.536A5 5 0 008 12m0 0a5 5 0 001.464 3.536" />
            ...
            <input
              ...
              className="flex-1 h-1 rounded-full appearance-none bg-gray-600 accent-blue-500 cursor-pointer"
              aria-label="Volume"
            />
          </div>
