...
function ChannelEmptyState({ hasWorkspace, canCreate, onCreateChannel }: {
  hasWorkspace: boolean;
  canCreate: boolean;
  onCreateChannel: () => void;
}) {
  if (!hasWorkspace) {
    return (
      <div className="px-3 py-6 flex flex-col items-center text-center gap-3">
        ...
        <p className="text-xs text-gray-500 leading-relaxed">Select a workspace to see channels</p>
      </div>
    );
  }
  ...
  return (
    <div className="px-3 py-6 flex flex-col items-center text-center gap-3">
      ...
      <div>
        <p className="text-xs font-medium text-gray-400 mb-1">No channels yet</p>
        {canCreate && (
          <button
      ...
      <div className="flex flex-col items-center gap-1 py-2">
        ...
        {canManage && hasWorkspace && (
          <SidebarTooltip label="Create channel">
            <button
              onClick={onCreateChannel}
              className="w-9 h-9 rounded-xl bg-white/5 hover:bg-indigo-500/20 text-gray-500 hover:text-indigo-400 flex items-center justify-center transition-all duration-150 mx-auto"
            >
              <PlusIcon />
            </button>
          </SidebarTooltip>
        ...
        <div className="space-y-0.5 px-1.5">
          {channels.map((channel) => (
            <div key={channel.id} className="relative group/ch flex items-center">
              ...
              >
                <HashIcon />
                <span className={`flex-1 truncate ${channel.unreadCount && channel.unreadCount > 0 ? 'font-semibold text-white' : ''}`}>
                  {channel.name}
                </span>
                <AnimatePresence>
                  {channel.unreadCount && channel.unreadCount > 0 ? (
