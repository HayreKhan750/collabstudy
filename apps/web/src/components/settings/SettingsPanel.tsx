...
function SaveButton({ loading, label = 'Save changes' }: { loading: boolean; label?: string }) {
  return (
    <button
      type="submit"
      disabled={loading}
      className="w-full sm:w-auto px-5 py-3 sm:py-2.5 bg-gradient-to-br from-violet-600 to-purple-700 hover:from-violet-500 hover:to-purple-600 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed text-white text-sm font-semibold rounded-xl transition-all duration-200 shadow-[0_0_16px_rgba(139,92,246,0.3)] hover:shadow-[0_0_24px_rgba(139,92,246,0.5)] min-h-[44px]"
    >
...
function AppearanceTab() {
  // Use next-themes as the single source of truth for theme state.
  // This eliminates the dual-system conflict (manual localStorage + next-themes)
  // that caused the UI to break until page refresh on every toggle.
  ...
  // Derive local theme state from next-themes (never from localStorage directly)
  const theme = (mounted ? (currentTheme as Theme) : 'dark') ?? 'dark';

  const applyTheme = (t: Theme) => {
    // next-themes handles localStorage + classList atomically — no manual DOM
    setNextTheme(t);
    setSaved(true);
    ...
    <div className="flex flex-col h-full bg-white/80 dark:bg-transparent text-slate-900 dark:text-white">
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="max-w-4xl mx-auto px-4 py-6 md:py-10 flex flex-col md:flex-row gap-4 md:gap-8">
          ...
          {/* Content */}
          <div className="flex-1 min-w-0 bg-white/95 dark:bg-[#0A0B10]/80 backdrop-blur-md border border-slate-200/80 dark:border-white/[0.06] rounded-2xl p-5 sm:p-8">
